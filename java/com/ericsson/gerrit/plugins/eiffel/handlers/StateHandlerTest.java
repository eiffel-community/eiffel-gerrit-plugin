...
public class StateHandlerTest {
    ...
    public void testBuildParentFilePath() throws Exception {
        ...
        Mockito.when(dbHandler.getEventID(Table.SCM_TABLE, BRANCH)).thenReturn("");
        stateHandler.setLastSentChangeMergedEiffelEvent(projectName , BRANCH, "someID");
    ...
    public void testgetLastSentEvent() throws Exception {
        ...
        String eventId = stateHandler.getLastSentChangeMergedEiffelEventId(PROJECT, BRANCH);
        assertEquals("Table.SCM_TABLE tabler key should be", "eventID", eventId);

        Mockito.when(dbHandler.getEventID(Table.SCM_TABLE, ""))
                .thenThrow(new ConnectException("Exception thrown by test"));
        stateHandler.getLastSentChangeMergedEiffelEventId("", "");

    }

    @Test
    public void testUpdateLastEventCalled() throws Exception {
        Mockito.when(dbHandler.getEventID(Table.SCM_TABLE, BRANCH)).thenReturn("old-event-id");

        String eventId = "event-id";
        stateHandler.setLastSentChangeMergedEiffelEvent(PROJECT, BRANCH, eventId);
        Mockito.verify(dbHandler).updateInto(Table.SCM_TABLE, BRANCH, eventId);
    ...
    public void testInsertLastEventCalled() throws Exception {
        ...
        Mockito.verify(dbHandler).insertInto(Table.SCM_TABLE, BRANCH, eventId);

    }

    @Test
    public void testConnectionErrorsHandled() throws Exception {
        Mockito.when(dbHandler.getEventID(Table.SCM_TABLE, PROJECT)).thenThrow(new ConnectException("Test Exception"));
        ...
        assertEquals("Error occured return empty string.", "", stateHandler.getLastSentChangeMergedEiffelEventId("", ""));
